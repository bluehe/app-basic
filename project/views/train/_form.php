...
<div class="row">
    <div class="col-md-12">
        ...
        <?= $form->field($model, 'train_start')->widget(DateTimePicker::classname(), [
        ...
        <?= $form->field($model, 'train_end')->widget(DateTimePicker::classname(), [
            'options' => ['placeholder' => '', 'autocomplete' => 'off'],
            'removeButton' => false,
            'pluginOptions' => [
                'autoclose' => true,
        ...
        <?= $form->field($model, 'sa')->widget(Select2::classname(), ['data' => $model->group_id ? User::get_role('sa', User::STATUS_ACTIVE, UserGroup::get_group_userid($model->group_id)) : [], 'options' => ['prompt' => '', 'multiple' => true,], 'maintainOrder' => true]); ?>

        <?= $form->field($model, 'other')->widget(Select2::classname(), ['data' => $model->group_id ? User::get_role('other', User::STATUS_ACTIVE, UserGroup::get_group_userid($model->group_id)) : [], 'options' => ['prompt' => '', 'multiple' => true,], 'maintainOrder' => true]); ?>
...
<script>
    ...
    $(function() {
        ...
        $('#train-corporation_id').change(function() {
            ...
            if (v) {
                $('.field-train-train_name').hide();
                $.getJSON("<?= Url::toRoute('common/corporation-info') ?>", {
                    id: v
                }, function(data) {
                    $('#train-other').val(data.bd).trigger('change');
                    $('#train-train_address').val(data.address);
                });
            } else {
                $('.field-train-train_name').show();
        ...
        $('#train-group_id').change(function() {
            ...
            if (v) {
                $.getJSON("<?= Url::toRoute(['common/group-corporation']) ?>", {
                    id: v
                }, function(data) {
                    $("select#train-corporation_id").html(data.corporation).trigger('change');
                    $("select#train-sa").html(data.sa).trigger('change');
                    $("select#train-other").html(data.other).trigger('change');
                });
            } else {

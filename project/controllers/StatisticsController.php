...
class StatisticsController extends Controller {
    ...
    public function actionCorporation() {
        ...
        if ($sum == 1) {
            ...
            //月
            $amount_num_start = ($allocate_total ? strtotime(key($allocate_total)) : strtotime(date("Y-m", $start))); //下拨开始日期
            $cloud_num_start = ($cloud_total ? strtotime(key($cloud_total)) : ($allocate_total ? strtotime(key($allocate_total)) : strtotime(date("Y-m", $start)))); //补贴开始日期
...
//        $items[$fk]=$fv;
    ...
    public function actionHealth() {
        ...
        $start = strtotime('-1 months +1 days', $end);
        $group = Yii::$app->request->get('group', null);
        $total_get = Yii::$app->request->get('total', 1); //全体或个人
        ...
        if ($total_get == 1) {
            ...
            foreach ($health_total as $total) {
                ...
                $activity_value[$key][$total['bd_id']]['week'] = isset($activity[$total['statistics_time']][$total['bd_id']]['week']) ? (int) $activity[$total['statistics_time']][$total['bd_id']]['week'] : 0;
            }
            asort($health_key);
            foreach ($health_value as $date => $value) {
                foreach ($health_key as $key) {
                    ...
                    $data_activity['week'][$key][] = ['name' => $date, 'y' => $activity_value[$date][$key]['week'], 'value' => [$date, $activity_value[$date][$key]['week']]];
                    $data_per_week[$key][] = ['name' => $date, 'y' => $sum == 0 ? 0 : round($activity_value[$date][$key]['week'] * 100 / $sum, 2), 'value' => [$date, $sum == 0 ? 0 : round($activity_value[$date][$key]['week'] * 100 / $sum, 2)]];
                }
            }
            ...
            if ($chart == 1) {
                ...
                if ($data_activity['week']) {
                    foreach ($data_activity['week'] as $gid => $data) {
                        $series['activity'][] = ['type' => 'column', 'name' => $gid && isset($groups[$gid]['name']) ? $groups[$gid]['name'] : '未分配', 'data' => $data, 'color' => $gid && isset($groups[$gid]['color']) ? '#' . $groups[$gid]['color'] : '#FF0000', 'stacking' => false];
                    }
                    foreach ($data_per_week as $gid => $per) {
                        $series['activity'][] = ['type' => 'spline', 'name' => $gid && isset($groups[$gid]['name']) ? $groups[$gid]['name'] : '未分配', 'data' => $per, 'tooltip' => ['valueSuffix' => '%'], 'yAxis' => 1, 'color' => $gid && isset($groups[$gid]['color']) ? '#' . $groups[$gid]['color'] : '#FF0000'];
                    }
                } else {
                    $series['activity'][] = [];

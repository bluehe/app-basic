...
class CorporationMeal extends \yii\db\ActiveRecord
{
    ...
    public static function get_num_base($start = '', $annual = null, $group_id = null)
    {
        if (!$group_id) {
            $group_id = UserGroup::get_user_groupid(Yii::$app->user->identity->id);
        }
        return static::find()->andWhere(['group_id' => $group_id])->andFilterWhere(['<', 'start_time', $start])->andFilterWhere(['annual' => $annual])->count();
    }
    ...
    public static function get_allocate_num($start = '', $end = '', $annual = null, $group_id = null)
    {
        ...
        return static::find()->andWhere(['group_id' => $group_id])->andFilterWhere(['and', ['>=', 'start_time', $start], ['<=', 'start_time', $end]])->andFilterWhere(['annual' => $annual])->select(['amount', 'num' => 'count(*)'])
            //->orderBy(['num' => SORT_DESC])
            ->groupBy(['amount'])->asArray()->all();
    }

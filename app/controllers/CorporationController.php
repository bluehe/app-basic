...
//                throw $e;
    ...
    public function actionCorporationUpdate($id) {
        ...
        $model->scenario='industry';
        if($model !== null&&Yii::$app->user->can('公司修改',['id'=>$id])){
...
//                throw $e;
            ...
            Yii::$app->session->setFlash('error', '企业不存在。');
            return $this->redirect(Yii::$app->request->referrer);
        }
    }
    ...
    public function actionCorporationView($id) {

        $model = Corporation::findOne($id);

        if ($model !== null) {
            return $this->renderAjax('corporation-view', [
                            'model' => $model,
            ]);
        } else {
            Yii::$app->session->setFlash('error', '企业不存在。');

...
class Menu extends \yii\widgets\Menu {
    ...
    protected function isItemActive($item) {
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            ...
            //multi-action 多action激活 高亮状态
            if (isset($item['options']) && isset($item['options']['multi-action']) && $item['options']['multi-action']) {
                if (is_array($item['options']['multi-action']) && in_array($arrayThisRoute[1], $item['options']['multi-action'])) {
                    return true;
                }
                if (is_string($item['options']['multi-action']) && $arrayThisRoute[1] === $item['options']['multi-action']) {
                    return true;
                }
            }
            ...
            if (isset($item['options']) && isset($item['options']['controller']) && $item['options']['controller']) {

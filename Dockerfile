FROM centos:latest
MAINTAINER bluehe 179611207@qq.com
...
#     && sed -i "64a             root           html/app/web;" /etc/nginx/nginx.conf \
#     && sed -i "64a             location ~ \.php$ {" /etc/nginx/nginx.conf \
#     && sed -i "3a daemon off;" /etc/nginx/nginx.conf \
#     && echo "<?php phpinfo()?>" > /usr/local/nginx/html/index.php \
#     && sed -i '45s/html;/html\/app\/web;/g' /etc/nginx/nginx.conf \
...
#        sleep 5s \n\
#        /usr/bin/mysql --connect-expired-password -e \"set password=password('\$MYSQL_PASSWORD');update mysql.user set host='%' where user='root' && host='localhost';flush privileges;create database app DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;\" \n\
...
#RUN rm -rf html && git clone https://github.com/bluehe/app-basic.git html \
#     && cd html \
...
#     && sed -i "6s/'dsn' => 'mysql:host=localhost;dbname=yii2advanced'/'dsn' => 'mysql:host=127.0.0.1;dbname=app'/g" common/config/main-local.php \

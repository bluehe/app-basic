...
class CrontabController extends Controller
{
    ...
    public function actionGitExec()
    {
        ...
        $left_day = $w > 0 && $w < 6 ? ($left_num - floor($gitexec_sum / 5 * (5 - $w))) : $left_num; //当天剩余执行数
        $left_hour = $left_day > 0 ? ($H >= 8 && $H < 18 ? ($left_day - floor($gitexec_sum / 5 / 10 * (17 - $H))) : $left_day) : 0;
    ...
    public function actionProjectClean()
    {
        ...
        if (count($corporation_ids) > 0) {
            foreach ($corporation_ids as $corporation_id) {
                CorporationProject::project_delete($corporation_id) ? Yii::info($corporation_id . '清理成功', 'projectclean') : Yii::warning($corporation_id . '清理失败', 'projectclean');
            }
        }
        return ExitCode::OK;
    }
